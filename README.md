...
Codex Island is a floating status island for the top of macOS. It stays compact by default and expands on hover to show Codex account limits, profile identity, lifetime and recent token activity, plus the source, compact model name, reasoning effort, Fast status, and cumulative token usage of the three most recent threads.
...
> These screenshots are generated by the built-in offscreen preview renderer. Limits, token counts, plan, dates, and thread content shown in them are demonstration data. The `≈ … Token` value is a personalized estimate derived from the previous seven complete calendar days, the current quota-window length, and the remaining percentage; it is not an exact Token allowance returned by Codex.
...
- Remaining primary rate-limit percentage, next reset time, reset-relative usage pace, and a personalized Token estimate based on the previous 7 complete days of usage
